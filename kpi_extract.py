    ...
    while (timepoint < ending_time):
        ...
        if (preset_start.shape[0]>0):
            ...
            if (preset_stop.shape[0]>0):
                ...
                # Check if "towing_status" is in the movement data
                if ("towing_status" not in df_tow.columns) or (df_tow.towing_status.isnull().sum()>0):
                    # find all entries of start_towing and stop_towing
                    start_towing_list = df[(df.datetime>=preset_start.datetime) & (df.datetime<=preset_stop.datetime) & (df.status=="start_towing")].reset_index()
                    stop_towing_list = df[(df.datetime>=preset_start.datetime) & (df.datetime<=preset_stop.datetime) & (df.status=="stop_towing")].reset_index()
                    timepoint_list = [preset_start.datetime] + sorted(list(start_towing_list.datetime) + list(stop_towing_list.datetime)) + [preset_stop.datetime]
                    total_towing_time = 0
                    total_time = (preset_stop.datetime - preset_start.datetime).total_seconds()
                    if (len(timepoint_list)==3) and (start_towing_list.shape[0]==1):
                        total_towing_time = (timepoint_list[2] - timepoint_list[1]).total_seconds()
                    elif (len(timepoint_list)==3) and (stop_towing_list.shape[0]==1):
                        total_towing_time = (timepoint_list[1] - timepoint_list[0]).total_seconds()
                    elif (len(timepoint_list)>3) and (start_towing_list.iloc[0].datetime <stop_towing_list.iloc[0].datetime):
                        for i in range(2, len(timepoint_list), 2):
                            total_towing_time += (timepoint_list[i] - timepoint_list[i-1]).total_seconds()
                    elif (len(timepoint_list)>3) and (start_towing_list.iloc[0].datetime > stop_towing_list.iloc[0].datetime):
                        for i in range(1, len(timepoint_list), 2):
                            total_towing_time += (timepoint_list[i] - timepoint_list[i-1]).total_seconds()
                else:
                ...
                output["牽引あり時間(s)"].append(int(total_towing_time))
                output["牽引なし時間(s)"].append(int(total_time - total_towing_time))
...
if (fleet_order is not None) and (fleet_movement is not None):
